...
Hosting, account deletion, per-account storage limits, owner-requested cleanup and Turnstile are deployed. Before accepting public users, finish live billing/pricing, hosted acceptance, production email, backups, monitoring and privacy/terms. The owner’s old notes were recovered; a general local importer remains separate work. Configure the host's security headers and HTTPS. Clearing an account in the Auth dashboard cascades database rows; private storage objects still require server-side cleanup. The Settings deletion flow is deployed; follow `ACCOUNT_DELETION.md` and test only with disposable accounts.
