...
test('configured CAPTCHA gates authentication and forwards one-use tokens for every public auth action', async () => {
  const { default: Turnstile } = await import('../src/components/Auth/Turnstile')
  const calls: any[] = []
  const auth = Object.fromEntries(['signInWithPassword','signUp','resetPasswordForEmail','resend'].map(name => [name, async (...args: any[]) => { calls.push([name,...args]); return {error:null} }])) as any
  act(() => { root = create(<CloudAuthForm recovery={false} authenticated={false} onRecovered={() => {}} auth={auth} captchaSiteKey="test-key" />) })
  fill('email','student@example.test'); fill('password','long passphrase 123')
  await submit()
  assert.equal(calls.length,0)
  assert.ok(text().includes('Complete the security verification'))
  act(() => root!.root.findByType(Turnstile).props.onToken('signin-token'))
  await submit()
  assert.equal(calls[0][1].options.captchaToken,'signin-token')
  await submit()
  assert.equal(calls.length,1,'consumed token must not be reused')
  act(() => button('Create account').props.onClick())
  fill('password','long passphrase 123'); fill('password','long passphrase 123',1)
  act(() => root!.root.findByType(Turnstile).props.onToken('signup-token'))
  await submit()
  assert.equal(calls[1][1].options.captchaToken,'signup-token')
  act(() => button('Resend confirmation email').props.onClick())
  act(() => root!.root.findByType(Turnstile).props.onToken('resend-token'))
  await submit()
  assert.equal(calls[2][1].options.captchaToken,'resend-token')
  act(() => button('Back to sign in').props.onClick())
  act(() => button('Forgot password?').props.onClick())
  act(() => root!.root.findByType(Turnstile).props.onToken('reset-token'))
  await submit()
  assert.equal(calls[3][2].captchaToken,'reset-token')
})

test('expired CAPTCHA and mode changes discard tokens; rejected requests require a fresh challenge', async () => {
  const { default: Turnstile } = await import('../src/components/Auth/Turnstile')
  let calls=0
  const auth={signInWithPassword:async()=>{calls++;return {error:new Error('bad credentials')}}} as any
  act(() => { root=create(<CloudAuthForm recovery={false} authenticated={false} onRecovered={()=>{}} auth={auth} captchaSiteKey="test-key" />) })
  act(()=>root!.root.findByType(Turnstile).props.onToken('expired'))
  act(()=>root!.root.findByType(Turnstile).props.onToken(''))
  await submit();assert.equal(calls,0)
  act(()=>root!.root.findByType(Turnstile).props.onToken('old-mode'))
  act(()=>button('Forgot password?').props.onClick())
  act(()=>button('Back to sign in').props.onClick())
  await submit();assert.equal(calls,0)
  act(()=>root!.root.findByType(Turnstile).props.onToken('fresh'))
