  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (mode === 'signin') {
        ...
        const { error } = await auth.resend({ type: 'signup', email: email.trim(), options: { emailRedirectTo: redirectTo } })
        if (error) throw error
  ...
  return <main className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
    <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        <button className="btn-primary w-full py-3 rounded-xl font-semibold disabled:opacity-50" disabled={busy}>{busy ? 'Please wait…' : mode === 'signin' ? 'Sign in' : mode === 'signup' ? 'Create account' : mode === 'forgot' ? 'Send reset link' : mode === 'confirm' ? 'Send confirmation link' : 'Save password'}</button>
      </form>
