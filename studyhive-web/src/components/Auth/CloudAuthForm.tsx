...
type AuthActions = Pick<SupabaseClient['auth'], 'signInWithPassword' | 'signUp' | 'resetPasswordForEmail' | 'updateUser'>

export default function CloudAuthForm({ recovery, authenticated, onRecovered, auth = cloudClient!.auth }: { recovery: boolean; authenticated: boolean; onRecovered: () => void; auth?: AuthActions }) {
  const [mode, setMode] = useState<'signin' | 'signup' | 'forgot' | 'reset'>(recovery && authenticated ? 'reset' : recovery ? 'forgot' : 'signin')
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (mode === 'signin') {
        ...
        const { error } = await auth.signUp({ email: email.trim(), password, options: { data: { username: username.trim() }, emailRedirectTo: redirectTo } })
        ...
        const { error } = await auth.resetPasswordForEmail(email.trim(), { redirectTo: `${redirectTo}?auth=recovery` })
        if (error) throw error
  ...
  return <main className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
    <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        {mode !== 'forgot' && <label className="block">Password<input className="w-full mt-1 px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary" type="password" autoComplete={mode === 'signin' ? 'current-password' : 'new-password'} value={password} onChange={e => setPassword(e.target.value)} required minLength={mode === 'signin' ? 1 : 12} maxLength={128} /></label>}
        ...
        <button className="btn-primary w-full py-3 rounded-xl font-semibold disabled:opacity-50" disabled={busy}>{busy ? 'Please wait…' : mode === 'signin' ? 'Sign in' : mode === 'signup' ? 'Create account' : mode === 'forgot' ? 'Send reset link' : 'Save password'}</button>
      </form>
